  ...
  const content = `
    <div class="task-details-modal">
      ...
      <div class="task-info-grid" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 15px; margin-bottom: 20px;">
        <div class="info-item">
          <div style="color: #9CA3AF; font-size: 12px; margin-bottom: 5px; text-transform: uppercase; letter-spacing: 0.5px;">Candidate</div>
          <div style="color: #E5E7EB; font-size: 15px; font-weight: 500;">${escapeHtml(task.candidate_name || task.candidateName || 'Unknown')}</div>
        </div>
        <div class="info-item">
...
export function openUnassignModal(taskId) {
  ...
  const displayClient = task?.candidate_name || task?.candidateName ? ` (${escapeHtml(task.candidate_name || task.candidateName)})` : '';
