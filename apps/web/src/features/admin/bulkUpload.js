  ...
  const content = `
    <div class="smart-upload-tabs" style="display:flex; flex-direction:column; gap:20px;">
      ...
      <div id="tab-text" class="tab-content" style="display:none">
        ...
        <div class="form-hint" style="color:#9ca3af; font-size:12px; margin-top:5px;"><i class="fas fa-info-circle"></i> Supports Tab, Comma, or Pipe (|) separators</div>
      ...
      <div id="tab-ocr" class="tab-content" style="display:none">
        <div class="file-drop-zone" id="ocrDropZone" style="border:2px dashed #374151; border-radius:12px; padding:40px; text-align:center; cursor:pointer;">
          ...
          <p style="color:#9ca3af; font-size:13px; margin-bottom:20px;">AI will extract text from the image</p>
...
// 3. Intelligent Column Mapper (Legacy Precision)
function smartColumnMapper(rawData) {
  ...
  rawData.forEach(row => {
    ...
    newRow.title = findValue('title')?.toString().trim() || `CASE-${Math.floor(Math.random()*100000)}`;
    newRow.clientName = findValue('clientName')?.toString().trim();
    newRow.pincode = findValue('pincode')?.toString().trim().replace(/[^0-9]/g, '');
    newRow.mapUrl = findValue('mapUrl')?.toString().trim();
    newRow.notes = findValue('notes')?.toString().trim();
    
    const empIdStr = findValue('assignedTo')?.toString().trim();
    if (empIdStr && state.allEmployees) {
      const emp = state.allEmployees.find(e => 
        e.employee_id?.toLowerCase() === empIdStr.toLowerCase() || 
...
// 4. Preview Window (Editable Table)
  ...
  const content = `
    <div class="preview-container">
      <div class="preview-header" style="margin-bottom:15px; display:flex; justify-content:space-between; align-items:center;">
        ...
        <span class="info-badge" style="color:#6366F1;"><i class="fas fa-edit"></i> Edit fields if needed</span>
      </div>
          ...
          <tbody>
            ${tasks.map((t, idx) => `
              <tr data-idx="${idx}" style="border-bottom:1px solid #334155;">
                ...
                <td style="padding:12px;">${t.assignedTo ? '<span style="color:#10B981; font-size:12px;"><i class="fas fa-user-check"></i> Matched</span>' : '<span style="color:#9CA3AF; font-size:12px;"><i class="fas fa-layer-group"></i> Pool</span>'}</td>
              </tr>
            `).join('')}
          </tbody>
        </table>
      </div>
      <div class="modal-actions" style="margin-top:20px; display:flex; gap:10px; justify-content:flex-end; border-top: 1px solid #1F2937; padding-top: 15px;">
...
export async function submitFinalBulkUpload() {
  ...
  rows.forEach(row => {
    ...
    task.title = row.querySelector('input[name="title"]').value.trim();
    task.pincode = row.querySelector('input[name="pincode"]').value.trim();
  ...
  try {
    ...
    updatedTasks.forEach(t => {
      ...
      if (existingMap.has(cleanTitle)) {
        duplicates.push({ ...t, existingId: existingMap.get(cleanTitle).id });
      } else {
        newTasks.push(t);
      }
    });
  ...
  const content = `
    <div style="display:flex; flex-direction:column; gap:20px;">
      <div class="form-info" style="background:rgba(245,158,11,0.1); border:1px solid rgba(245,158,11,0.3); padding:15px; border-radius:8px; display:flex; gap:12px; align-items:center;">
        ...
        <span style="color:#e5e7eb;"><strong>${duplicates.length}</strong> duplicate case(s) found.</span>
      </div>

      <div style="max-height:150px; overflow-y:auto; background:#111827; padding:15px; border-radius:8px; border:1px solid #374151;">
        <div style="color:#9ca3af; font-size:12px; margin-bottom:8px; font-weight:600; text-transform:uppercase;">Duplicates:</div>
...
export async function handleBulkDuplicateChoice(choice) {
  ...
  if (choice === 'create') {
    showToast(`Finalizing ${ctx.duplicates.length + ctx.newTasks.length} imports...`, 'info');
    await processBulkUpload([...ctx.duplicates, ...ctx.newTasks]);
  } else if (choice === 'update') {
    showToast('Updating duplicates and creating new...', 'info');
    await updateExistingTasks(ctx.duplicates);
    await processBulkUpload(ctx.newTasks);
  } else if (choice === 'cancel') {
    showToast(`Importing ${ctx.newTasks.length} new tasks...`, 'info');
    await processBulkUpload(ctx.newTasks);
  }
...
async function updateExistingTasks(duplicates) {
  ...
  showToast(`${success} task(s) updated successfully`, 'success');
}
