...
// ≡ƒÜ¿ Battery Saver: Track discrete actions only. Removing 'scroll' and 'touchmove' stops the 
...
// ≡ƒÜ¿ Zero-Latency Background GPS Engine: 
...
if (navigator.geolocation && currentUser && currentUser.role !== 'admin') {
  navigator.geolocation.watchPosition(
    ...
    { enableHighAccuracy: false, timeout: 10000, maximumAge: 30000 } // ≡ƒÜ¿ Switched to false to stop severe battery drain
  );
}

// 5. UTILITY FUNCTIONS
// ≡ƒÜ¿ HYPER-OPTIMIZED: Uses pure Regex instead of creating heavy DOM nodes
...
function closeAllModals() {
  ...
  modals.forEach(modal => {
    ...
    // ≡ƒÜ¿ Memory Leak Proof: Only remove if the node actually still exists in the DOM
    setTimeout(() => { 
      if (modal && modal.parentNode) modal.remove(); 
    }, 300);
  });
  
  // ≡ƒÜ¿ Restore Background Scrolling
  const contentArea = document.querySelector('.content-area');
  if (contentArea) contentArea.style.overflowY = 'auto';
  
  // ≡ƒÜ¿ Prevent Mobile RAM Leaks by destroying the ghost listeners
...
function createModal(title, content, options = {}) {
  ...
  // ≡ƒÜ¿ Lock Background Scrolling (Stops Scroll Bleed)
  const contentArea = document.querySelector('.content-area');
...
function cleanupCurrentView() {
  ...
  // ≡ƒÜ¿ Prevent Map Memory Leaks: Destroy Leaflet engine if it exists in background
...
function initMenu() {
  console.log('≡ƒöº initMenu() called');
  ...
  if (!menu) {
    console.error('Γ¥î CRITICAL: sidebarMenu element NOT FOUND in HTML!');
    return;
  }
  
  // Clear existing menu
  menu.innerHTML = '';
  console.log('Γ£ô Menu cleared');
  
  if (currentUser.role === 'admin') {
    console.log('Γ£ô Building ADMIN menu...');
  ...
  button.innerHTML = `<i class="fas ${btn.icon}"></i> <span>${btn.text}</span>`;  // ΓåÉ WRAPPED IN SPAN!
  ...
  console.log(`Γ£ô Button ${index + 1} created:`, btn.text);
});

// Set first button as active by default
menu.querySelector('.menu-item')?.classList.add('active');
    
    console.log('Γ£ô All admin menu buttons created');
    ...
    console.log('Γ£ô Building EMPLOYEE menu...');
...
menu.querySelector('.menu-item')?.classList.add('active');
    ...
    console.log('Γ£ô All employee menu buttons created');
  ...
  console.log('≡ƒÄë initMenu() complete!');
}
  ...
  let html = `
    ...
    <style>
      ...
      .form-input {
        ...
        border-radius: 10px;
        color: #E5E7EB;
        font-size: 16px; /* ≡ƒÜ¿ 16px absolutely prevents the iOS Safari Auto-Zoom Trap */
  ...
  if (mapUrlInput) {
    mapUrlInput.addEventListener('input', function() {
      ...
      if (url) {
        ...
        // Γ£à FIX: Warn admin that the short-link requires manual coordinates
      ...
      fetch('/api/tasks', {
        ...
        .then(data => {
          ...
          if (data.success) {
            showToast(
              formData.assignedTo 
                ? 'Γ£ô Task assigned successfully!' 
                : 'Γ£ô Task created as unassigned!', 
              'success'
            );
...
function showTodayTasks() {
  ...
  let html = `
    ...
    <style>
      ...
      @media (hover: hover) {
        ...
        /* ≡ƒÜ¿ Map Button Hardware Acceleration */
        .map-quick-btn:hover {
          background: rgba(59, 130, 246, 0.3) !important;
          border-color: #3B82F6 !important;
        }
      }
      /* ≡ƒÜ¿ DOM RAM Saver: Moved inline styles here to shrink HTML payload by 70% */
      ...
      .mobile-optimized-card:active {
        transform: scale(0.98) translateZ(0) !important; /* ≡ƒÜ¿ Keeps card locked to GPU during tap */
        background: #1e293b !important;
      }

      /* ≡ƒÜ¿ Map Button Clean CSS */
      .map-quick-btn {
      ...
      .map-quick-btn:active {
        transform: scale(0.95) translateZ(0); /* ≡ƒÜ¿ Keeps button locked to GPU during tap */
      }
    </style>
  ...
  // ≡ƒÜ¿ Debounce Search: Wait 300ms after typing stops before rebuilding UI
  let searchTimeout;
...
function loadTodayTasks(searchTerm) {
  ...
  fetch(url, {
    ...
    .then(tasks => {
      ...
      // ≡ƒôì [LOAD-TASKS] Debug: Log coordinate state of tasks from API
      ...
      console.log(`≡ƒôì [LOAD-TASKS] Received ${rawTasks.length} tasks from API:`);
      console.log(`≡ƒôì [LOAD-TASKS]   ${withDbCoords} with DB lat/lng, ${withMapUrl} with map_url, ${withPincode} with known pincode`);
      rawTasks.forEach(t => {
        const hasCoords = parseFloat(t.latitude) && parseFloat(t.longitude);
        const hasUrl = t.map_url || t.mapUrl;
        console.log(`≡ƒôì [LOAD-TASKS]   Task ${t.id} "${t.title}": lat=${t.latitude}, lng=${t.longitude}, map_url=${hasUrl ? 'YES' : 'NULL'}, pincode=${t.pincode}, coordsInDB=${hasCoords ? 'YES' : 'NO'}`);
      });
      ...
      // ≡ƒÜ¿ Persist the exact Elite route sequence if active
      // ≡ƒÜ¿ Persist the exact Elite route sequence if active
...
// ≡ƒÜ¿ Ghost Virtual Scroll: Only renders cards in the viewport to maintain 60FPS
...
function displayEmployeeTasks(tasks) {
  if (window.cardObserver) window.cardObserver.disconnect(); // ≡ƒÜ¿ Explicitly clear memory to prevent mobile RAM leaks over long shifts
  const list = document.getElementById('todayTasksList');
  if (!list) return; // Γ£à Prevents background crashes when on the Map tab
  ...
  // ≡ƒÜ¿ V8 Engine Optimization: Uses Array.map and join() instead of RAM-heavy string concatenation
...
const approxBadge = isApprox
  ...
  // ≡ƒÜ¿ Hardware Acceleration: Allow UI animations to finish painting before locking the CPU to render HTML
  requestAnimationFrame(() => {
    if (list) {
      list.innerHTML = headerHtml + tasksHtml;
      // ≡ƒÜ¿ Attach virtual scroll observer to new DOM nodes
...
function updateTaskStatus(taskId) {
  ...
  // Helper to send update (≡ƒÜ¿ Added Offline Resilience Queue)
  const sendUpdate = (coords = {}) => {
    ...
    .catch(err => {
      ...
      // ≡ƒÜ¿ Push to Offline Queue to prevent data loss in basements/elevators
      ...
      // ≡ƒÜ¿ Auto-sync when network is restored
      if (!window._offlineSyncAttached) {
        window.addEventListener('online', async () => {
          ...
          if (queue.length > 0) {
            ...
            if (failCount > 0) {
              showToast(`ΓÜá∩╕Å ${failCount} tasks failed to sync`, 'warning');
            } else {
              showToast('Γ£ô All offline tasks synced!', 'success');
            }
  ...
  if (newStatus === 'Completed') {
    if (lastKnownLocation) {
      // ≡ƒÜ¿ Instant submission from background tracker (0ms UI freeze)
...
function reapplyDistanceSorting(tasks, userLat, userLng) {
  console.log(`≡ƒôì [DIST-SORT] Sorting ${tasks.length} tasks from user position [${userLat}, ${userLng}]`);
  ...
  let pool = tasks.map(t => {
    ...
    // Γ£à FIX: On-the-fly extraction for sorting
    if (isNaN(lat) || isNaN(lng) || lat === 0) {
      ...
      if (link) {
        ...
        // ≡ƒôì DEBUG: Check if !3d/!4d coords differ
        const m3d = link.match(/!3d(-?[0-9.]+)/);
        const m4d = link.match(/!4d(-?[0-9.]+)/);
        if (m3d && m4d && match) {
          const diffLng = Math.abs(parseFloat(match[2]) - parseFloat(m4d[1]));
          if (diffLng > 0.001) {
            console.warn(`≡ƒôì [DIST-SORT] ΓÜá∩╕Å Task ${t.id} "${t.title}": @ lng=${match[2]} vs !4d lng=${m4d[1]} ΓåÆ DIFF ${(diffLng*111320).toFixed(0)}m`);
          }
        }
      }
    }
    ...
    console.log(`≡ƒôì [DIST-SORT] Task ${t.id} "${t.title}": [${lat}, ${lng}] source=${coordSource}`);
  ...
  navigator.geolocation.getCurrentPosition(
    async (pos) => {
      try {
        ...
        console.log(`≡ƒôì [ORS-ENRICH] Enriching ${allEmployeeTasks.length} tasks for ORS optimization...`);
        const enrichedTasks = allEmployeeTasks.map(t => {
          ...
          // Γ£à FIX: On-the-fly extraction for Elite Routing Engine
          if (!lat || !lng) {
            const link = t.map_url || t.mapUrl || t.mapurl;
            if (link) {
              const match = link.match(/@(-?[0-9.]+),(-?[0-9.]+)/) || link.match(/\?q=(-?[0-9.]+),(-?[0-9.]+)/);
              ...
              // ≡ƒôì DEBUG: Check if !3d/!4d actual place coords exist and differ
              const m3d = link.match(/!3d(-?[0-9.]+)/);
              const m4d = link.match(/!4d(-?[0-9.]+)/);
              if (m3d && m4d) {
                console.log(`≡ƒôì [ORS-ENRICH] Task ${t.id} "${t.title}": !3d/!4d ACTUAL=[${m3d[1]}, ${m4d[1]}]`);
                if (match) {
                  const diffLng = Math.abs(parseFloat(match[2]) - parseFloat(m4d[1]));
                  const diffLat = Math.abs(parseFloat(match[1]) - parseFloat(m3d[1]));
                  if (diffLng > 0.001 || diffLat > 0.001) {
                    console.warn(`≡ƒôì [ORS-ENRICH] ΓÜá∩╕Å Task ${t.id}: DISCREPANCY @=[${match[1]},${match[2]}] vs !3d/4d=[${m3d[1]},${m4d[1]}] ΓåÆ ${(Math.max(diffLat,diffLng)*111320).toFixed(0)}m apart!`);
                  }
                }
              }
            }
          ...
          console.log(`≡ƒôì [ORS-ENRICH] Task ${t.id} "${t.title}": FINAL=[${lat}, ${lng}] source=${coordSource}`);
          return { ...t, _lat: lat, _lng: lng };
        });
        ...
        if (result.success && result.optimizedTasks) {
          ...
          showToast('Γ£ô Route optimized successfully!', 'success');
        } else {
    ...
    { enableHighAccuracy: true, timeout: 10000, maximumAge: 30000 } // ≡ƒÜ¿ 30s cache for instant loads
  );
}
...
function loadAllTasks() {
  ...
  fetch(url)
    .then(res => res.json())
    .then(tasks => {
      // Γ£à FIX 1: Use 'allTasks' (matches the global variable at the top)
      allTasks = Array.isArray(tasks) ? tasks : [];
      
      const filtered = applyDateFilter(allTasks); 
      
      // Γ£à FIX 2: Save 'currentFilteredTasks' so pagination knows what list to use
...
function displayAllTasksList(tasks) {
  ...
  // ---------------------------------------------------------
  // ≡ƒºá LOGIC: Pagination & Filtering
  // ---------------------------------------------------------
  ...
  // ---------------------------------------------------------
  // ≡ƒÄ¿ UI: Header & Table Construction
  // ---------------------------------------------------------
  let html = `
...
<button class="btn-icon btn-secondary" onclick="openUnassignModal(${t.id}, '${escapeHtml(t.title).replace(/&#039;/g, "\\'")}', '${escapeHtml(t.clientName || '').replace(/&#039;/g, "\\'")}')" title="Unassign"><i class="fas fa-times"></i></button>            <button class="btn-icon btn-danger" onclick="deleteTask(${t.id})" title="Delete"><i class="fas fa-trash"></i></button>
  ...
  // ---------------------------------------------------------
  // ≡ƒª╢ UI: Pagination Footer (New Feature)
  // ---------------------------------------------------------
  if (totalPages > 1) {
    html += `
  ...
  // ---------------------------------------------------------
  // ≡ƒö¿ UI: Bulk Actions & Styling (Restored from your Original)
  // ---------------------------------------------------------
  html += `
...
function loadUnassignedTasks() {
  ...
  console.log('≡ƒôí Fetching unassigned tasks from:', url);
  ...
  fetch(url, {
    ...
    .then(res => res.json())
    .then(tasks => {
      console.log('Γ£ô Received', tasks.length, 'unassigned tasks');
    ...
    .then(r => r.json())
    .then(employees => {
      console.log('Γ£ô Received', employees.length, 'employees');
      displayUnassignedList(allUnassignedTasks, employees);
    })
    .catch(err => {
      console.error('Γ¥î Error loading tasks:', err);
...
function saveEditedMapUrl(taskId) {
  ...
  if (!input) {
    console.error('Γ¥î editMapUrl input not found!');
    return;
  }
  
  const url = input.value.trim();
  
  // Γ£à FIX: Grab the values from the new Lat/Lng boxes
  const latVal = document.getElementById('editLatitude').value;
  const lngVal = document.getElementById('editLongitude').value;
  
  console.log('≡ƒÆ╛ Saving map URL for task', taskId);
  ...
  fetch(`/api/tasks/${taskId}`, {
    ...
    body: JSON.stringify({ 
      map_url: url,
      // Γ£à FIX: Send the coordinates to your newly updated server route
    ...
    .then(data => {
      ...
      if (data.success) {
        showToast('Γ£ô Map URL updated successfully!', 'success');
        closeAllModals();
        ...
        setTimeout(() => {
          ...
          if (document.getElementById('unassignedTasksList')) {
            console.log('≡ƒöä Refreshing unassigned tasks view...');
            loadUnassignedTasks();
          }
          if (document.getElementById('allTasksList')) {
            console.log('≡ƒöä Refreshing all tasks view...');
            loadAllTasks();
          }
        }, 300);
    ...
    .catch(err => {
      console.error('Γ¥î Error updating map URL:', err);
      showToast('Connection error. Please try again.', 'error');
      if (saveBtn) {
        saveBtn.disabled = false;
        saveBtn.innerHTML = '<i class="fas fa-save"></i> Save Changes';
...
function showAddEmployee() {
  ...
  document.getElementById('addEmpForm').addEventListener('submit', function(e) {
    ...
    fetch('/api/users', {
      ...
      .then(data => {
        if (data.success) {
          showToast('Γ£ô Employee created successfully!', 'success');
          closeAllModals();
...
function deleteEmployeePrompt(id, name) {
  if (!confirm(`ΓÜá∩╕Å DELETE EMPLOYEE: ${name}\n\nThis is permanent. Continue?`)) return;
  const pass = prompt('≡ƒöÉ ADMIN PASSWORD REQUIRED:');
  if (!pass) return;
  ...
  const content = `
    ...
    <div style="background: rgba(239, 68, 68, 0.1); border: 1px solid rgba(239, 68, 68, 0.3); border-radius: 8px; padding: 15px; margin-bottom: 20px;">
      ...
      <div style="max-height: 150px; overflow-y: auto; color: #D1D5DB; font-size: 12px;">
        ${duplicates.map(d => `ΓÇó ${escapeHtml(d.title)}`).join('<br>')}
      </div>
    </div>
...
async function processBulkUpload(tasks) {
  ...
  try {
    ...
    showToast(`Γ£ô ${successCount} task(s) created successfully!`, 'success');
...
function showEditEmployeeModal(empId) {
  ...
  document.getElementById('editEmpForm').addEventListener('submit', function(e) {
    ...
    fetch(`/api/users/${empId}`, {
      ...
      .then(data => {
        if (data.success) {
          showToast('Γ£ô Employee updated successfully!', 'success');
          closeAllModals();
          showEmployees(); // Refresh list
        } else {
          showToast(data.message || 'Update failed', 'error');
...
// ≡ƒÜ¿ Deprecated synchronous version of showActivityLog removed to prevent hoisting conflicts and memory issues
...
async function showActivityLog() {
  ...
  try {
    ...
    logs.forEach(log => {
      ...
      // Γ£à FIX: Handle different Action colors
...
function confirmReassign(taskId) {
  ...
  fetch(`/api/tasks/${taskId}/assign`, {
    ...
    .then(data => {
      if (data.success) {
        showToast('Γ£ô Task reassigned successfully!', 'success');
        closeAllModals();
        // Refresh current view
        if (document.getElementById('allTasksList')) loadAllTasks();
        if (document.getElementById('unassignedTasksList')) loadUnassignedTasks();
...
function confirmUnassign(taskId) {
  fetch(`/api/tasks/${taskId}`, {
    ...
    .then(data => {
      if (data.success) {
        showToast('Γ£ô Task moved to unassigned pool', 'success');
        closeAllModals();
        if (document.getElementById('allTasksList')) loadAllTasks();
      } else {
...
async function confirmBulkAssign() {
  ...
  showToast(`Γ£ô ${successCount} assigned, ${failCount} failed`, successCount > 0 ? 'success' : 'error');
...
async function bulkDeleteTasks() {
  ...
  if (!confirm(`ΓÜá∩╕Å Delete ${selected.length} task(s)?\n\nThis action cannot be undone!`)) {
    return;
  }
  ...
  showToast(`Γ£ô ${successCount} deleted, ${failCount} failed`, successCount > 0 ? 'success' : 'error');
  clearSelection();
...
async function confirmStatusUpdate(taskId) {
  ...
  try {
    ...
    if (result.success) {
      ...
      showToast(`Γ£ô Status updated to ${newStatus}`, 'success');
...
// 2. Intelligent Column Mapper (Γ¡É Features: Any Name, Any Order, Pincode Extraction)
function smartColumnMapper(rawData) {
  ...
  rawData.forEach(row => {
    ...
    // Γ¡É UPDATED: Robust Employee Matching (ID OR Name)
    // ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
...
// 3. Smart Preview Modal (≡ƒæü∩╕Å Features: Editable Table, Live Save)
// 3. Smart Preview Modal (≡ƒæü∩╕Å Features: Editable, Loading State, Duplicate Check)
function showSmartPreview(tasks) {
  ...
  // Γ¡É UPDATED LOGIC: Capture Edits -> Loading State -> Check Duplicates -> Upload
...
async function showMapRouting() {
  ...
  // ≡ƒÜ¿ Elite Map Engine Recycling: Only build DOM if map doesn't exist to prevent RAM spikes
  ...
  if (!window.L && !window._isMapLoading) {
    window._isMapLoading = true; // ≡ƒÜ¿ Locks the thread to prevent double-tap injection crashes
    await new Promise(resolve => {
      ...
      script.onload = () => { window._isMapLoading = false; resolve(); }; // ≡ƒÜ¿ Releases the lock
      document.head.appendChild(script);
    });
  }
  ...
  navigator.geolocation.getCurrentPosition(
    (pos) => {
      ...
      // ≡ƒÜ¿ CRITICAL FIX: Prevent fatal crash if the executive switched tabs while GPS was loading
      ...
      if (!window.routingMapInstance) {
        ...
        // ≡ƒÜ¿ Butter Smooth: Instantly wipe old markers without killing the map engine
      ...
      L.marker([userLat, userLng])
        ...
        .bindPopup('<b style="color:#2563eb; font-size:14px;">≡ƒôì You Are Here</b>')
        .openPopup();
      ...
      // ≡ƒÜ¿ Batch Rendering: Create a background array to hold markers
      const mapMarkers = [];

      console.log(`≡ƒôì [MAP-ROUTING] Plotting ${activeTasks.length} active tasks on map...`);
      ...
      activeTasks.forEach((t, index) => {
        ...
        // Γ£à FIX: On-the-fly extraction if database coords are missing
        if (!lat || !lng) {
          const link = t.map_url || t.mapUrl || t.mapurl;
          if (link) {
            const match = link.match(/@(-?[0-9.]+),(-?[0-9.]+)/) || link.match(/\?q=(-?[0-9.]+),(-?[0-9.]+)/);
            ...
            // ≡ƒôì DEBUG: Check for !3d/!4d discrepancy
            const m3d = link.match(/!3d(-?[0-9.]+)/);
            const m4d = link.match(/!4d(-?[0-9.]+)/);
            if (m3d && m4d && match) {
              const diffLng = Math.abs(parseFloat(match[2]) - parseFloat(m4d[1]));
              const diffLat = Math.abs(parseFloat(match[1]) - parseFloat(m3d[1]));
              if (diffLng > 0.001 || diffLat > 0.001) {
                console.warn(`≡ƒôì [MAP-ROUTING] ΓÜá∩╕Å Pin ${index+1} Task ${t.id} "${t.title}": COORDS WRONG! Using @=[${match[1]},${match[2]}] but REAL !3d/4d=[${m3d[1]},${m4d[1]}] ΓåÆ ${(Math.max(diffLat,diffLng)*111320).toFixed(0)}m off!`);
        ...
        console.log(`≡ƒôì [MAP-ROUTING] Pin ${index+1}: Task ${t.id} "${t.title}" ΓåÆ [${lat}, ${lng}] source=${coordSource} map_url=${(t.map_url||t.mapUrl) ? 'YES' : 'NULL'}`);

        if (lat && lng) {
          waypoints.push([lat, lng]);
          
          // Γ£à FIX: Make the map pin Orange if it's using the inaccurate Pincode fallback
      ...
      console.log(`≡ƒôì [MAP-ROUTING] SUMMARY: ${mapCoordStats.db} from DB, ${mapCoordStats.atViewport} from @viewport, ${mapCoordStats.qQuery} from ?q=, ${mapCoordStats.pincodeFallback} from pincode, ${mapCoordStats.noCoords} with NO coords, ${mapCoordStats.discrepancies} with @/!3d discrepancies`);

      // ≡ƒÜ¿ Batch Rendering: Stamp all markers onto the map perfectly simultaneously
    ...
    { enableHighAccuracy: true, timeout: 15000, maximumAge: 30000 } // ≡ƒÜ¿ 30s cache for instant loads
  );
}
...
document.addEventListener('DOMContentLoaded', () => {
  console.log('Γ£ô Dashboard loaded, initializing...');
  console.log('Current user:', currentUser);
  
  // Verify user is authenticated
  if (!currentUser || !currentUser.role) {
    console.error('Γ¥î No user found, redirecting to signin...');
  ...
  if (userName) {
    ...
    console.log('Γ£ô User name updated:', currentUser.name);
  } else {
    console.error('Γ¥î userName element not found');
  }
  
  if (userRole) {
    userRole.textContent = currentUser.role;
    console.log('Γ£ô User role updated:', currentUser.role);
  } else {
    console.error('Γ¥î userRole element not found');
  }
  ...
  if (userInitials) {
    ...
    console.log('Γ£ô User initials updated:', initials);
  } else {
    console.error('Γ¥î userInitials element not found');
  }
  
  // Initialize the menu and default view
  console.log('Γ£ô Calling initMenu() for role:', currentUser.role);
  initMenu();
  ...
  console.log('Γ£ô Dashboard initialization complete!');
});
